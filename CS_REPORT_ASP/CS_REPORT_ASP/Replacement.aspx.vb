...
Public Class Replacement
    ...
    Protected Sub gvItemList_RowDeleting(sender As Object, e As GridViewDeleteEventArgs)
        ...
        If Not String.IsNullOrEmpty(txtReplacementNo.Text.Trim()) Then
            Using conn As New SqlConnection(ConStr)
                conn.Open()
                Using cmd As New SqlCommand("DELETE FROM [CS_DATA].[dbo].[REPLACEMENT_D] WHERE RP_NO = @RP_NO AND ITM_CD = @ITM_CD", conn)
                    cmd.Parameters.AddWithValue("@RP_NO", txtReplacementNo.Text.Trim())
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        Try
            Dim ReplacementNo As String = txtReplacementNo.Text.Trim()

            If String.IsNullOrWhiteSpace(ReplacementNo) Then
                lblStatus.Text = "Please input Replacement Note number"
                txtReplacementNo.Focus()
                Exit Sub
            End If
            ...
            Using conn As New SqlConnection(ConStr)
                ...
                Dim checkSql As String = "SELECT COUNT(*) FROM [CS_DATA].[dbo].[REPLACEMENT_H] WHERE RP_NO = @RP_NO"
                Using checkCmd As New SqlCommand(checkSql, conn)
                    checkCmd.Parameters.AddWithValue("@RP_NO", ReplacementNo)
                ...
                If dlExists Then
                    ' --- UPDATE HEADER ---
                    Dim updateSql As String = "UPDATE [CS_DATA].[dbo].[REPLACEMENT_H] SET " &
                        "RP_DT = @RP_DT, CS_CD = @CS_CD, SOLD_TO = @SOLD_TO, CS_ADDRESS = @CS_ADDRESS, " &
                        "TAX_ID = @TAX_ID, BRANCH_NO = @BRANCH_NO, ATTN_NM = @ATTN_NM, TEL_NO = @TEL_NO, " &
                        "RP_TKR_NM = @RP_TKR_NM, RP_UPDATE = @RP_UPDATE, LOC_SEND = @LOC_SEND " &
                        "WHERE RP_NO = @RP_NO"

                    Using cmd As New SqlCommand(updateSql, conn)
                        cmd.Parameters.AddWithValue("@RP_NO", ReplacementNo)
                        cmd.Parameters.AddWithValue("@RP_DT", Convert.ToDateTime(txtDate.Text))
                        ...
                        cmd.Parameters.AddWithValue("@RP_TKR_NM", "")
                        cmd.Parameters.AddWithValue("@RP_UPDATE", Now)
                        cmd.Parameters.AddWithValue("@LOC_SEND", txtShipPoint.Text.Trim())
                        cmd.ExecuteNonQuery()
                    End Using

                    ' ลบรายการเก่าก่อนเพิ่มใหม่
                    Dim deleteSql As String = "DELETE FROM [CS_DATA].[dbo].[REPLACEMENT_D] WHERE RP_NO = @RP_NO"
                    Using delCmd As New SqlCommand(deleteSql, conn)
                        delCmd.Parameters.AddWithValue("@RP_NO", ReplacementNo)
                        delCmd.ExecuteNonQuery()
                    End Using

                Else
                    ' --- INSERT HEADER ---
                    Dim insertSql As String = "INSERT INTO [CS_DATA].[dbo].[REPLACEMENT_H] " &
                        "(RP_NO, RP_DT, CS_CD, SOLD_TO, CS_ADDRESS, TAX_ID, BRANCH_NO, ATTN_NM, TEL_NO, LOC_SEND) " &
                        "VALUES (@RP_NO, @RP_DT, @CS_CD, @SOLD_TO, @CS_ADDRESS, @TAX_ID, @BRANCH_NO, @ATTN_NM, @TEL_NO, @LOC_SEND)"
                ...
                For Each row As DataRow In dt.Rows
                    Dim detailSql As String = "INSERT INTO [CS_DATA].[dbo].[REPLACEMENT_D] " &
                        "(RP_NO, ITM_CD, ITM_NM, EX_ITM_NM, PO_NO, QTY, UNIT_PRICE, REMARK) " &
                        "VALUES (@RP_NO, @ITM_CD, @ITM_NM, @EX_ITM_NM, @PO_NO, @QTY, @UNIT_PRICE, @REMARK)"
    ...
    Protected Sub btnEdit_Click(sender As Object, e As EventArgs) Handles btnEdit.Click
        Dim ReplacementNo As String = txtRNSearch.Text.Trim()

        If String.IsNullOrEmpty(ReplacementNo) Then
        ...
        Try
            Using MyConn As New SqlConnection(ConStr)
                ...
                ' Header
                Dim sqlHeader As String = "SELECT * FROM [CS_DATA].[dbo].[REPLACEMENT_H] WHERE RP_NO = @RP_NO"
                Dim cmdHeader As New SqlCommand(sqlHeader, MyConn)
                cmdHeader.Parameters.AddWithValue("@RP_NO", ReplacementNo)
                ...
                If dtHeader.Rows.Count = 0 Then
                    lblMessage.Text = "No data found for Replacement Note Number " & ReplacementNo
                    ClearForm()
                    Return
                End If
                ...
                txtReplacementNo.Text = rowHeader("RP_NO").ToString()
                If IsDate(rowHeader("RP_DT")) Then
                    txtDate.Text = Convert.ToDateTime(rowHeader("RP_DT")).ToString("yyyy-MM-dd")
                End If
                ...
                ' Detail
                Dim sqlDetail As String = "SELECT * FROM [CS_DATA].[dbo].[REPLACEMENT_D] WHERE RP_NO = @RP_NO"
                Dim cmdDetail As New SqlCommand(sqlDetail, MyConn)
                cmdDetail.Parameters.AddWithValue("@RP_NO", ReplacementNo)
